...
6. ~~Tasarım implementasyonu kısmi~~ — **TAMAMLANDI**: `AppShell`/`Sidebar`, Dashboard/Çalışanlar halka grafikleri, Takvim'e gün değiştirme (ok + gün şeridi, `/takvim?date=YYYY-MM-DD`) ve personel bazlı günlük doluluk yüzdesi de eklendi. Ayrıca kullanıcı geri bildirimiyle: Çalışanlar'daki "Yeni Personel" formu artık kapalı başlıyor (Müşteriler'deki gibi).
